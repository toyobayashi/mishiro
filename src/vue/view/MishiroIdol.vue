...
<div class="main-block-style">
  <div class="gray-area idol-result pull-left margin-right-20">
    <ul>
      <li :class="{active:activeCard.name === i.name}" :key="i.id" v-for="i in searchResult" v-text="i.name" @click="selectedIdol(i)"></li>
    </ul>
  </div>
  <div>
    ...
    <button class="cgss-btn cgss-btn-star pull-right margin-left-10" @click="downloadVoice" :disabled="voiceDisable" >{{$t("idol.voiceBtn")}}</button>
  ...
  <div class="black-bg idol-info margin-top-10">
    ...
    <table class="table-bordered" border="1" :class="{
      ...
      <tr v-if="activeCard.limited">
        ...
        <td colspan="3"><p v-for="limit in activeCard.limited" :key="limit.id">{{"(" + (limit.id > 9999 ? $t("idol.gacha") + limit.id : $t("idol.event") + limit.id) + ") " + limit.name + ": " + limit.startDate + " ～ " + limit.endDate}}</p></td>
      </tr>
    </table>
  </div>
</div>
