...
//   public static init () {
//     keyBind()
//     const canvasLive = document.getElementById('live') as HTMLCanvasElement
//     Game.CTX = canvasLive.getContext('2d') as CanvasRenderingContext2D

//     const canvasIconBar = document.getElementById('iconBar') as HTMLCanvasElement
...
//     const name = parse(song.src).name.split('-')[1]
//     document.getElementsByTagName('title')[0].innerHTML = liveResult.name = name
//     liveResult.fullCombo = song.fullCombo
//     const music = process.env.NODE_ENV === 'production' ? new Audio(song.src) : new Audio(relative(window.preload.getPath('public'), song.src))
...
//   private static readonly IMG_POSITION_X: number = 0
//   private static readonly IMG_POSITION_Y: number = 0
//   t: number
...
//         miss()
//       } else if (self.exist) {
//         const d = Game.TOP_TO_BOTTOM - self.y
...
//   const dt = Math.abs(note.y - Game.TOP_TO_BOTTOM) / Game.PX_SPEED
//   if (dt <= Game.RANGE) {
//     note.exist = false
...
//   se.longLoop.pause()
//   const dt = Math.abs(note.y - note.length - Game.TOP_TO_BOTTOM) / Game.PX_SPEED
//   if (dt <= Game.RANGE) {
//     note.exist = false
//     cancelAnimationFrame(note.t)
//     note.clean()
...
//   const note = Game.queue[path - 1][0]
//   if (note) {
